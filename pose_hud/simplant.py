...
start_realtime() で背景スレッドが dt 間隔で実時間ステップし、制御ループを
無改造で回せる(ループが壁時計で測る dt とシミュレータの dt が一致する)。
...
class SimulatedVRChat:
    ...
    def __init__(
        ...
        *,
        ...
        self._cmds = {
            a: _DelayedCommand(
                model.axes[a].deadtime_s if a in model.axes else 0.0
            )
