...
def run_move_probe(
    ...
    *,
    ...
    deadline = rec.t0 + wait_cap
    pose = reader.get_latest()
    while pose is None:
        if monotonic() > deadline:
            raise RuntimeError(f"no pose before move probe (axis={axis})")
        sleep(poll)
        pose = reader.get_latest()
...
def build_plant(
    ...
    """複数軸のプローブ記録から PlantModel を組む。"""
    axes = {r.axis: identify_axis(r) for r in runs}
    dts: list[float] = []
    for r in runs:
