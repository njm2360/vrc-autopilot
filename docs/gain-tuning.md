...
- 全ループはヘッドレス実行可: `SimulatedVRChat`(app/sysid/sim_plant.py)が reader/look/move の3役、`SimClock` を clock に渡すと仮想時間で高速に回る。呼び出しパターンは tests/test_sysid.py と app/cli/sim_face.py が実例。
...
sim-face と patrol-buttons はゲインフラグが共通で、指標も同じ計算。実機で face の yaw_err が tol 前後で往復し turn が ±0.50 を行き来していたら face yaw の振動。turn_p/turn_i/turn_d の内訳でどの項が暴れているか分かる。**sim と実機で食い違ったらゲインでなくモデルを疑う**(静特性の再プロット、むだ時間、そして sim に無い慣性やノイズの有無)。look をマウス入力にする場合は不感帯が無いので deadzone 0、ゲインは別物として再調整。
